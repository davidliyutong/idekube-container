...
The project uses Make + shell scripts. Build arguments are read from `.dockerargs` (a symlink to `.dockerargs.base` or `.dockerargs.ascend`), and env vars from `.env` (symlink to `.env.base` or `.env.ascend`).
...
- **`agent/`** — Standalone agent toolchain (openclaw + Claude Code + opencode + document processing) exposing only `/agent` and `/ssh`. Variants: `base`

### Directory Layout

- `manifests/docker/<flavor>/<variant>/Dockerfile` — Dockerfiles for each image variant
- `manifests/install-scripts/` — Modular shell scripts invoked by Dockerfiles (e.g., `setup-packages.sh`, `setup-code-server.sh`, `setup-vnc.sh`)
...
3. Normal mode — configures shell, home directory, SSH keys, runs startup hooks from `/etc/idekube/startup.bash/*.sh`, then starts supervisord
